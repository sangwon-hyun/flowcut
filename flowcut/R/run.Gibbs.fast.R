  ...
  ## Build censored box
   ...
   ##### prior specifications
    if(is.null(user.prior)){
        ...
        ## nu1=p+1 
        S0 = diag(dimdat)*(nu0-dimdat-1) * (1.5 / qnorm(0.995))^2 
        ## S1=diag(p+1)
        ## a_gamma <- gamma.ab[1]  
        ## b_gamma <- gamma.ab[2]
        ...
        nu0 <- user.prior$nu0 
        S0 <- user.prior$S0 
        invNugget <- 1 / user.prior$sig2_gamma  
        prior.spec.list <- list(nu0 = nu0,
                                S0 = S0,
                                sig2_gamma = sig2_gamma, 
                                gg = gg )        
...
################################################
        if( ( jj > tt.impute | !is.null(last.para)) & !is.null(Cbox)){
            censored.Z.list <- parallel::mcmapply(function(crow,zz){ zz[crow] },
                                                  crow = censor.which.list, zz=Z.list,
                                                  mc.cores = n.cores)  
   ...
   ################################################ 
        ...
        for(ell in 1:numclust){
            ...
            Sig.ell[,,ell] <- matrixsampling::rinvwishart(1, nu00 + m.ell[ell], 
